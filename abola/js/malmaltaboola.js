// ==UserScript==
// @name         Isolation test: block ONLY Yieldlove v2 loader
// @description  Blocks only https://cdn-a.yieldlove.com/v2/yieldlove.js (incl. querystring); allows everything else (Taboola runs freely). Blue background indicator.
// @author       HP
// @version      1.3
// @match        *://www.abola.pt/*
// @match        *://*.abola.pt/*
...
(() => {
  ...
  // -------- Block rule: ONLY Yieldlove v2 loader --------
  // Matches:
  //  - https://cdn-a.yieldlove.com/v2/yieldlove.js
  //  - https://cdn-a.yieldlove.com/v2/yieldlove.js?abola.pt
  //  - ...any query/hash variants
  const BLOCK_RX = /^https:\/\/cdn-a\.yieldlove\.com\/v2\/yieldlove\.js(?:[?#].*)?$/i;

  function log(kind, detail) {
    try { console.warn('[MITM-YL-BLOCK]', kind, detail); } catch (_) {}
  }

  function normalizeUrl(raw) {
    try {
      const u = new URL(String(raw), location.href);
      // We keep query because we want to match it too; strip hash only for stability
      u.hash = '';
      return u.toString();
    } catch (_) {
      return null;
    ...
    window.fetch = function(input, init) {
      try {
        if (isBlocked(input)) {
          ...
          return Promise.reject(new Error('Blocked Yieldlove v2 loader by MITM'));
        }
      } catch (_) {}
  ...
  // -------- 3) Purge any already-present Yieldlove loader script tags --------
  ...
  log('active', 'Blocking ONLY cdn-a.yieldlove.com/v2/yieldlove.js (Taboola runs freely).');
})();
