...
export default class ImaPlugin {
    ...
    initAdObjects() {
        ...
        this.adDisplayContainer = new window.google.ima.AdDisplayContainer(this.adContainer, this.player.media);
        this.adsLoader = new window.google.ima.AdsLoader(this.adDisplayContainer);
        this.adsLoader.addEventListener(window.google.ima.AdsManagerLoadedEvent.Type.ADS_MANAGER_LOADED, this.onAdsManagerLoaded, false);
        this.adsLoader.addEventListener(window.google.ima.AdErrorEvent.Type.AD_ERROR, this.onAdError, false);
    ...
    onAdsManagerLoaded(adsManagerLoadedEvent) {
        const adsRenderingSettings = Object.assign(Object.assign(Object.assign({}, new window.google.ima.AdsRenderingSettings()), { uiElements: [
        ...
        this.adsManager.addEventListener(window.google.ima.AdEvent.Type.CONTENT_PAUSE_REQUESTED, this.onContentPauseRequested);
        this.adsManager.addEventListener(window.google.ima.AdEvent.Type.CONTENT_RESUME_REQUESTED, this.onContentResumeRequested);
        ...
        this.adsManager.addEventListener(window.google.ima.AdEvent.Type.ALL_ADS_COMPLETED, this.onAllAdsCompleted);
        ...
        if (Array.isArray(this.cuePoints) &&
    ...
    onAdStarted(e) {
        ...
        this.player.elements.outerContainer.classList[this.isLinearAd ? 'remove' : 'add']('v-adNonLinear');
    ...
    onAdError(e) {
        ...
        clearTimeout(this.timerAdTimeout);
        try {
            const { type, errorCode, errorMessage } = (_a = e.getError()) === null || _a === void 0 ? void 0 : _a.j;
    ...
    addCuePoints() {
        ...
        this.player.getDuration().then((duration) => {
            this.cuePoints
                .filter((cuePoint) => cuePoint !== 0 && cuePoint !== -1 && cuePoint < duration)
                .forEach((cuePoint) => {
    ...
    onPlayerPlay() {
        this.playerIsEnded = false;
        if (this.adsLoaded || this.adError || this.adTimeoutReached)
            return;
        ...
        try {
            this.adsManager.init(this.player.media.clientWidth, this.player.media.clientHeight, window.google.ima.ViewMode.NORMAL);
     ...
     */
    onVolumeChange() {
        this.player.getVolume().then((volume) => this.adsManager.setVolume(volume));
    }
    /**
    ...
    onPlayerEnterFullscreen() {
        if (this.adsManager) {
            this.adsManager.resize(window.screen.width, window.screen.height, window.google.ima.ViewMode.FULLSCREEN);
        }
    }
    /**
     * On player ended
    ...
    onPlayerExitFullscreen() {
        if (this.adsManager) {
            this.adsManager.resize(this.player.media.clientWidth, this.player.media.clientHeight, window.google.ima.ViewMode.NORMAL);
        }
    }
    /**
